...
public class RxAVFile: IAVQueryable {
    ...
    public func retore(serverState: IObjectState) -> RxAVFile {
        let file = RxAVFile()
        file.state = FileState(objectState: serverState)
        return file
    }

    public typealias AVQueryableType = RxAVFile
    ...
    public init() {
        ...
        self.state.app = RxAVClient.sharedInstance.takeApp(app: nil)
    }
    ...
    public static func text(name: String, content: String, encoding: String.Encoding) -> RxAVFile {
        let data = content.data(using: encoding)!
        let file = RxAVFile(name: name, data: data)
        return file
    }

    public static func query() -> RxAVQuery<RxAVFile> {
         return RxAVQuery<RxAVFile>(className: RxAVFile.defaultClassName)
    }
